## THIS SCIPT IS BASED ON bivFns_lik.r, USED FOR BIVARIATE ANALYSIS FOR REAL DATASETS FOR A SINGLE CYCLE, WITH CORRELATION FOR NOU.
...
matrixVi <- function(theta, ni, ti, process) {
  ...
  if (process == "OU") {
    ...
    G3 <- outer(ti, ti, nouCov, theta[16], theta[9:11], theta[13:15])
    Gammai <- GammaiFn(G1, G2, G3, ni)
...
nouCov <- function(x, y, rho, theta1, theta2) {
  logrho <- log(rho)
  resCov <- exp(logrho*abs(x-y) + (1/2)*(theta1[1] + theta2[1] 
                                         + theta1[2]*x + theta2[2]*y
                                         + theta1[3]*x^2 + theta2[3]*y^2))
  return(resCov)
}
...
# GammaiFn <- function(G1, G2, covM=matrix(0, ni, ni), ni) {
GammaiFn <- function(G1, G2, covM, ni) {
  # Merge matrices
  # G1offDiag <- matrix(rbind(G1, matrix(0, ni, ni)), nrow = ni)
  # G2offDiag <- matrix(rbind(matrix(0, ni, ni), G2), nrow = ni)
  
  G1offDiag <- matrix(rbind(G1, covM), nrow = ni)
  G2offDiag <- matrix(rbind(covM, G2), nrow = ni)
...
nouPcv1 <- function(x, y, rho, theta1, theta2) {# rho_3
  cv1 <- (abs(x-y)/rho) * nouCov(x, y, rho, theta1, theta2)
  return(cv1)
}

nouPcv2 <- function(x, y, rho, theta1, theta2) {# a10, a20
  cv2 <- (1/2) * nouCov(x, y, rho, theta1, theta2)
  return(cv2)
}

nouPcv3 <- function(x, y, rho, theta1, theta2) {# a11
  cv3 <- (x/2) * nouCov(x, y, rho, theta1, theta2)
  return(cv3)
}

nouPcv4 <- function(x, y, rho, theta1, theta2) {# a21
  cv4 <- (y/2) * nouCov(x, y, rho, theta1, theta2)
  return(cv4)
}

nouPcv5 <- function(x, y, rho, theta1, theta2) {# a12
  cv5 <- (x^2/2) * nouCov(x, y, rho, theta1, theta2)
  return(cv5)
}

nouPcv6 <- function(x, y, rho, theta1, theta2) {# a22
  cv6 <- (y^2/2) * nouCov(x, y, rho, theta1, theta2)
}
...
PVi <- function(theta, ni, ti, process) {
  ...
  if (process == "OU") {
    ...
    rho3 <- outer(ti, ti, nouPcv1, theta[16], theta[9:11], theta[13:15])
    ac10 <- outer(ti, ti, nouPcv2, theta[16], theta[9:11], theta[13:15])
    ac20 <- outer(ti, ti, nouPcv2, theta[16], theta[9:11], theta[13:15])
    ac11 <- outer(ti, ti, nouPcv3, theta[16], theta[9:11], theta[13:15])
    ac21 <- outer(ti, ti, nouPcv4, theta[16], theta[9:11], theta[13:15])
    ac12 <- outer(ti, ti, nouPcv5, theta[16], theta[9:11], theta[13:15])
    ac22 <- outer(ti, ti, nouPcv6, theta[16], theta[9:11], theta[13:15])
    
    pv_1rho <- GammaiFn(rho1, zeroMat, zeroMat, ni)
    pv_1a0 <- GammaiFn(a10, zeroMat, ac10, ni)
    pv_1a1 <- GammaiFn(a11, zeroMat, ac11, ni)
    pv_1a2 <- GammaiFn(a12, zeroMat, ac12, ni)
    ...
    pv_2rho <- GammaiFn(zeroMat, rho2, zeroMat, ni)
    pv_2a0 <- GammaiFn(zeroMat, a20, ac20, ni) 
    pv_2a1 <- GammaiFn(zeroMat, a21, ac21, ni)
    pv_2a2 <- GammaiFn(zeroMat, a22, ac22, ni) 
    
    pv_3rho <- GammaiFn(zeroMat, zeroMat, rho3, ni)
    
    pvlist <- list("pV_phi1i" = pV_phi1i, "pV_phi2i" = pV_phi2i, "pV_phi3i" = pV_phi3i, 
                   "pV_sigma1i" = pV_sigma1i, "pV_sigma2i" = pV_sigma2i, 
                   "pv_1rho" = pv_1rho, "pv_1a0" = pv_1a0, "pv_1a1" = pv_1a1, "pv_1a2" = pv_1a2, 
                   "pv_2rho" = pv_2rho, "pv_2a0" = pv_2a0, "pv_2a1" = pv_2a1, "pv_2a2" = pv_2a2, "pv_3rho" = pv_3rho)
    return(pvlist)
  } else if (process == "Wiener") {
...
fisher.scoring <- function(theta, X, Y, N1, N2, K, niVec, r, time, id, B1dou, B2dou, tol, cap, process, dim) {
  ...
  while (diff > tol) {
    ...
    if (process == "OU") {
      ...
      while (any(theta1[c(1:3, 5:8, 12, 16)] < 0)){
        theta1 <- (theta1 + theta0)/2
      }
    } else if (process == "Wiener") {
...
results <- function(data, response, fixed, random = cbind(1, 1), process, time, id, tol, cap) {
  ...
  if (process == "OU") { 
    ...
    theta[s:(s+8)] <- c(0.4, -1, 0.1, -0.1, 0.1, -2, 1, -0.1, 0.2)
    # Test whether OU == NOU
  ...
  if (test1(id, time) && test2(id, time)) {
    ...
    newlist <- list("theta" = newTheta, "betaHat" = res$betaHat, "f1Hat" = res$f1Hat, "f2Hat" = res$f2Hat, "sdBeta" = sqrt(diag(covBeta)), "sdF1" = sqrt(diag(covF1)), "sdF2" = sqrt(diag(covF2)), "biasBeta" = biasBeta, "biasF1" = biasF1, "biasF2" = biasF2, "conv" = fs$conv, "capConv" = fs$capConv, "loglik" = loglik, "AICc" = AICc, "AIC" = AIC, "sdTheta" = fs$sdTheta, "b" = res$b, "U" = res$U, "fitted" = res$fitted, "trueRes" = res$trueRes, "diagV" = diag(V))
    return(newlist)
  }
  
  newlist <- list("theta" = newTheta, "betaHat" = res$betaHat, "f1Hat" = res$f1Hat, "f2Hat" = res$f2Hat, "sdBeta" = sqrt(diag(covBeta)), "sdF1" = sqrt(diag(covF1)), "sdF2" = sqrt(diag(covF2)), "conv" = fs$conv, "capConv" = fs$capConv, "loglik" = loglik, "AICc" = AICc, "AIC" = AIC, "sdTheta" = fs$sdTheta, "b" = res$b, "U" = res$U, "fitted" = res$fitted, "trueRes" = res$trueRes, "diagV" = diag(V))
  return(newlist)
}