...
system.time(res <- results(data, response = cbind(log(data$adjpdg2), log(data$adje1c2)), fixed = cbind(data$age, data$underWeight, data$overWeight), random = cbind(1, 1), process = "NOU", time = data$standday, id = data$womanid, tol = 0.001, cap = 50))
...
write.csv(unlist(res), "LiuBiv100_newSE.txt")
