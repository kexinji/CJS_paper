...
helperFn <- function(data, time, id, fixed, response) {
  ...
  # RETURN
  return(list("niVec" = niVec, "K" = K, "N1" = N1, "N2" = N2, "X" = X, "Y" = Y, "r" = r, "B1dou" = B1dou, "B2dou" = B2dou, "tprime" = tprime))
}
...
est <- function(theta, X, Y, N1, N2, K, niVec, r, time, id, process) {
  ...
  resLik <- Y - meanComp
  py <- W %*% resLik
  lik <- (-log(det(V)) - t(resLik) %*% py - lambda1 * t(f1Hat) %*% K %*% f1Hat - lambda2 * t(f2Hat) %*% K %*% f2Hat)/2	
  #--------------------------------------------------------------------------
  # Estimations of b, U
  ...
  estimates <- list("betaHat" = betaHat, "f1Hat" = f1Hat, "f2Hat" = f2Hat, "WN1" = WN1, "WN2" = WN2, "C22" = C22, "C33" = C33, "invC" = invC, "V" = V, "W" = W, "py" = py, "lik" = lik, "b" = b, "U" = U, "fitted" = fitted, "residual" = residual)
  return(estimates)
}
