eachRes <- function(i) {
	# source("bivFns_sim.R")
  source("bivFns_3c.R")
  # source("bivFns_3c_shiftedS.R")
  # source("bivFns_3c_switch.R")
  
  
	# NOU
  # old initializations
	# theta <- c(rep(1, 3), -0.5, rep(1, 3), rep(c(0.1, -5, 1.5, -0.1), 2)) 
  theta <- c(rep(1, 3), -0.5, rep(1, 3), rep(c(0.1, -3, -5, -2), 2)) 
  
  # initializations from liu analysis
  # theta <- c(rep(1, 3), -0.5, rep(1, 3), 0.2, -0.44, 0.3, -0.2, 0.15, -1.6, 0.3, -0.1)
	
	# NOU testing for equality with OU
	# theta <- c(rep(1, 3), -0.5, rep(1, 3), rep(c(-log(0.25), -1.02, 0, 0), 2))
	
	set.seed(i)
	data <- newData(theta, beta = c(1, 0.75), 10, 28, process = "NOU", cycle = 2)
	
	# test <- data[which(data[,1]==1),3]
	# plot(test, type = "l")
	
	#------------------------------------------------------------------------------------
	# Obtain moadel parameter estimates using the specified parameters
	res <- results(data, response = cbind(data$Y1, data$Y2), fixed = data$age, random = cbind(1, 1), process = "NOU", time = data$day, id = data$id, tol = 0.001, cap = 50)
	
	write.csv(unlist(res), paste('NOU2cNS_10subj_' , i, '.txt', sep=''))
}

library(parallel)
cl <- makeCluster(100)

parLapply(cl, 1:600, eachRes)

# Once we are done we need to close the cluster so that resources such as memory are returned to the operating system.
stopCluster(cl)

#------------------------------------------------------------------------------------
# Loop

source("bivFns_3c.R")

for (i in 17:100) {
  cat(paste0("Simulation: ", i, "\n"))
  
  # NOU
  theta <- c(rep(1, 3), -0.5, rep(1, 3), rep(c(0.1, -3, -5, -2), 2)) 
  set.seed(i)
  data <- newData(theta, beta = c(1, 0.75), 10, 28, process = "NOU", cycle = 2)
  
  system.time(res <- results(data, response = cbind(data$Y1, data$Y2), fixed = data$age, random = cbind(1, 1), process = "NOU", time = data$day, id = data$id, tol = 0.001, cap = 10))
  
  #------------------------------------------------------------------------------------
  # Obtain moadel parameter estimates using the specified parameters
  # res <- results(data, response = cbind(data$Y1, data$Y2), fixed = data$age, random = cbind(1, 1), process = "NOU", time = data$day, id = data$id, tol = 0.001, cap = 10)
  
  write.csv(unlist(res), paste('NOU2c_10subj_' , i, '.txt', sep=''))
}


