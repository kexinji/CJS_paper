...
est <- function(theta, X, Y, N, K, niVec, r, time, id, process) {
	...
	nX <- ncol(X)
	newlist <- list("betaHat" = estimates[1:nX], "fHat" = matrix(estimates[(nX+1):(nX+r)]), "V" = V, "W" = W, "WX" = WX, "WN" = WN, "C11" = C11, "C22" = C22, "invC" = invC)
	return(newlist)
}
...
fisher.scoring <- function(par, X, Y, N, K, niVec, r, time, id, Bdou, tol, cap, process, dim) {
