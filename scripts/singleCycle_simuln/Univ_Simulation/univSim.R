...
findPar = function(theta, Y) {
	...
	# ERROR HERE IN FISHER INFO
	# DIDN'T TAKE ACCOUNT INTO THE SYMMETRY HERE, I.E. THE LOWER LEFT BLOCK IS MISSING
	# I_tauTau
...
# par is the parameter initialization
...
simulation = function(n, par, sim, totalNum, tol, cap) {
	...
	while (i < sim & count < totalNum) {
		...
		# Gaussian process
		tempU = rsOU(n*tp, c(0, par[4], par[5]))
		u = matrix(tempU, n, tp)
		...
		temp = fisher.scoring(par, Y, tol, cap)
	...
	return(list(beta.sim, f.sim, theta.sim, count))
}


# est = simulation(n, sim, theta0)
system.time(est <- simulation(n = 30, theta0, sim = 100, totalNum = 200, tol = c(0.003, 0.001, 0.001, 0.003, 0.01), cap = 50))
