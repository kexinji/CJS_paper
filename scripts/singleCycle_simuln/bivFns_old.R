...
helperFn <- function(data, time, id, fixed, response) {
	...
	# Q <- matrix(0, r, (r-2))
	# diag(Q) <- 1/h[1:(r-2)]
	# diag(Q[-1:-2, ]) <- 1/h[2:(r-1)]
	# diag(Q[c(-1, -r), ]) <- -1/h[1:(r-2)] - 1/h[2:(r-1)]
	
	# # R
	# dr <- (h[1:(r-2)] + h[2:(r-1)])/3
	# R <- diag(dr, (r-2))
	# diag(R[(2-r), -1]) <- h[2:(r-2)]/6
	# diag(R[-1, (2-r)]) <- h[2:(r-2)]/6	
...
# #--------------------------------------------------------------------------	
# Zhang et al method for matrix B	
# B <- Q %*% ginv(crossprod(Q))
...
est <- function(theta, X, Y, N1, N2, K, niVec, r, time, id, process) {
	...
	estimates <- list("betaHat" = res[1:nX], "f1Hat" = matrix(res[(nX+1):(nX+r)]), "f2Hat" = matrix(res[(nX+r+1):(nX+2*r)]), "WN1" = WN1, "WN2" = WN2, "C22" = C22, "C33" = C33, "invC" = invC, "V" = V, "W" = W)
	return(estimates)
}
...
PVi <- function(theta, ni, ti, process) {
	...
	if (process == "OU") {
		...
		pv_theta12i <- Gammai(parV_theta12i, zeroMat, ni)
		pv_theta13i <- Gammai(parV_theta13i, zeroMat, ni)
		
		parV_theta22i <- outer(ti, ti, ouPv1, theta[10], theta[11])
		parV_theta23i <- outer(ti, ti, ouPv2, theta[10], theta[11])
		pv_theta22i <- Gammai(zeroMat, parV_theta22i, ni)
		pv_theta23i <- Gammai(zeroMat, parV_theta23i, ni)
		...
		pv_1rho <- Gammai(rho1, zeroMat, ni)
		pv_1a0 <- Gammai(a10, zeroMat, ni)
		pv_1a1 <- Gammai(a11, zeroMat, ni)
		pv_1a2 <- Gammai(a12, zeroMat, ni)
		...
		pv_2rho <- Gammai(zeroMat, rho2, ni)
		pv_2a0 <- Gammai(zeroMat, a20, ni) 
		pv_2a1 <- Gammai(zeroMat, a21, ni)
		pv_2a2 <- Gammai(zeroMat, a22, ni) 
...
fisher.scoring <- function(par, X, Y, N1, N2, K, niVec, r, time, id, B1dou, B2dou, tol, cap, process, dim) {
	...
	while (any(diff > tol)) {
	    ...
	    theta0 <- theta1
		theta1 <- findPar(theta0, X, Y, N1, N2, K, niVec, r, time, id, B1dou, B2dou, process, dim)
		
		#--------------------------------------------------------------------------	
		# check whether theta is in parameter space
		if (process == "OU") {
			for (i in c(1:3, 5:11)){
				while (theta1[i]<0) {
		...
		# Generate a vector of indicator variables, 1 if converge; 0 otherwise. 
		conv <- as.numeric(diff <= tol)
		
		# Abort this iteration if iternum > cap, and set the parameters to be zero
		if (iternum > cap) {
			theta1 <- matrix(0, dim)
			return(list("theta" = theta1, "capConv" = iternum, "conv" = conv))
		}
		print(iternum)
		print (diff)
		print(theta1)
...
results <- function(data, response, fixed, random = cbind(1, 1), process, time, id, tol, cap) {
	...
	s <- s + 2
	if (process == "OU") { 
		# theta is initialized to ensure that rho = 0.8, var = 1
		theta[s] <- -log(0.0963) # 2.340287
		theta[s+1] <- sqrt(-log(0.0963)*2) # 2.163463
		theta[s+2] <- -log(0.0963) 
		theta[s+3] <- sqrt(-log(0.0963)*2) 
	} else if (process == "NOU") {
		theta[s:(s+7)] <- rep(c(-log(0.0963), -2.1563, 3.6710, -0.145), 2)
	}
	...
	if (identical(newTheta, matrix(0, dim))) {
		...
		if (test1(id, time) && test2(id, time)) {
			...
			newlist <- list("theta" = newTheta, "betaHat" = res$betaHat, "f1Hat" = res$f1Hat, "f2Hat" = res$f2Hat, "sdBeta" = sqrt(diag(covBeta)), "sdF1" = sqrt(diag(covF1)), "sdF2" = sqrt(diag(covF2)), "biasBeta" = biasBeta, "biasF1" = biasF1, "biasF2" = biasF2, "conv" = fs$conv, "capConv" = fs$capConv)
			return(newlist)
		}

		newlist <- list("theta" = newTheta, "betaHat" = res$betaHat, "f1Hat" = res$f1Hat, "f2Hat" = res$f2Hat, "sdBeta" = sqrt(diag(covBeta)), "sdF1" = sqrt(diag(covF1)), "sdF2" = sqrt(diag(covF2)), "conv" = fs$conv, "capConv" = fs$capConv)
		return(newlist)
	}
}