...
# sapply(1:28, function(v) return(rsOU(1, theta=c(0, theta[4], t3[v]))))
	...
	res <- results(simdata, simdata$Y, simdata$age, random = 1, process = "OU", simdata$day, simdata$id, tol = 0.001, cap = 50)
	
	write.csv(unlist(res), paste('univOU_t001c50_' , i, '.txt', sep=''))
}

library(parallel)
cl <- makeCluster(100)

parLapply(cl, 65:164, eachRes)
...
# fs <- fisher.scoring(theta, X, Y, N, K, niVec, r, time, id, Bdou, tol = 0.1, cap = 10, process, dim)