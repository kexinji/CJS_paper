...
fisher.scoring = function(lambda0, eps = rep(0.01, 5)) {
	...
	while(diff > eps) {
		lambda.old = lambda 
		betaHat = beta.est(lambda[1, 1])
		fHat = f.est(lambda[1, 1])
		Pstar = PstarFunction(lambda[1, 1])
		
		score = scoreFunction(lambda[1, 1]) 	# score function
		info = infoFunction(lambda[1, 1])		# information matrix 
